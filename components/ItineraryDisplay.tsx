        ...
        <div className="flex flex-wrap gap-2 w-full xl:w-auto ml-14 xl:ml-0 z-10">
          ...
          <div className="flex gap-2 flex-grow sm:flex-grow-0">
            <button
              onClick={onSave}
              disabled={isSaved}
              className={`flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg border transition-all shadow-sm ${isSaved
                    ...
                    {day.activities.map((activity, actIdx) => (
                      <div key={actIdx} className="relative flex flex-col md:flex-row gap-5 group activity-item" style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
                        ...
                        <div className={`flex-1 ${theme.cardBg} ${theme.cardBorder} p-5 hover:shadow-md transition-shadow relative overflow-hidden print:border print:border-slate-200 print:shadow-none print:bg-white`}>
                          {/* Type Color Strip */}
                          <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${activity.type === 'food' ? 'bg-orange-400' :
                          ...
                          <div className="flex justify-between items-start mb-3 pl-2">
                            <div className="flex items-center gap-3">
                              ...
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide border ${activity.type === 'food' ? 'bg-orange-50 text-orange-600 border-orange-100' :
